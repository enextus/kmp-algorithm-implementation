...
public class KMPApplication {
	...
	private static final int Y_BUTTON_POSITION = 680;
	...
	// Position for the radio buttons
	private static final int X_RADIO_BUTTON_POSITION = 120;
	private static final int Y_RADIO_BUTTON_POSITION = 620;
	// Other constants
	public static final String KMP_ALGORITHM = "Knuth-Morris-Pratt algorithm";
	public static final String BM_ALGORITHM = "Boyer-Moore algorithm";

	// Create a new instance of the KMPProvider algorithm class
	KMPProvider kmpProvider = new KMPProvider();
	BMProvider bmProvider = new BMProvider();
	...
	public KMPApplication() {
		...
		// Create a new JFrame, set its title to 'provider.KMPProvider Algorithm'
	...
	private void placeComponents(JPanel panel) {
		...
		// Create the radio buttons for selecting the algorithm
		JRadioButton kmpButton = new JRadioButton(KMP_ALGORITHM, true);
		JRadioButton bmButton = new JRadioButton(BM_ALGORITHM);
		kmpButton.setBounds(X_RADIO_BUTTON_POSITION, Y_RADIO_BUTTON_POSITION, BUTTON_WIDTH, BUTTON_HEIGHT);
		bmButton.setBounds(X_RADIO_BUTTON_POSITION, Y_RADIO_BUTTON_POSITION + BUTTON_HEIGHT, BUTTON_WIDTH, BUTTON_HEIGHT);

		// Create a group for the radio buttons
		ButtonGroup algorithmGroup = new ButtonGroup();
		algorithmGroup.add(kmpButton);
		algorithmGroup.add(bmButton);
		...
		searchButton.addActionListener(e -> {
			...
			if (kmpButton.isSelected()) result = kmpProvider.KMPAlgorithm(text, pattern);
			else result = bmProvider.BMAlgorithm(text, pattern);
